...
namespace EnClass.AssemblyCSharpImport
{
	public class NETImport
	{
		...
		public bool ImportAssembly(string fileName)
		{
			...
			{
				Debug.WriteLine($"{Strings.Error_NoAssembly}");
				return false;
			}
			try
			{
				...
				{
					Debug.WriteLine($"{Strings.UnsafeTypesPresent} {Strings.WarningTitle}");
				}
			}
			catch (ReflectionTypeLoadException ex)
			{
				Debug.WriteLine($"{Strings.Error_MissingReferencedAssemblies} {ex.GetType()} {ex.Message}");
				return false;
			}
			...
			{
				Debug.WriteLine($"{Strings.Error_BadImageFormat} {ex.GetType()} {ex.Message}");
				return false;
			}
			catch (Exception ex)
			{
